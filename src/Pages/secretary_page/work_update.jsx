...
const isTaskArchived = (task) => {
  ...
  return lines.some((line) => SECRETARY_ARCHIVED_TAG_RE.test(String(line || "")));
};

const isAdminArchivedTask = (task) => {
  const lines = String(task?.description || "").split(/\r?\n/);
...
export default function WorkUpdate() {
  ...
  const adminArchivedTaskIdSet = useMemo(
    () =>
      new Set(
        (Array.isArray(tasks) ? tasks : [])
          .filter((task) => isAdminArchivedTask(task))
          .map((task) => getTaskKey(task))
          .filter(Boolean)
      ),
    [tasks]
            ...
            {filteredTasks.map((t, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {secretarySteps.length > 0 && (
                    <div
                      className={`mt-3 rounded-lg border px-3 py-2 ${
                        pendingSecretarySteps.length > 0
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div
                            className={`text-[11px] uppercase tracking-wide ${
                              pendingSecretarySteps.length > 0 ? "text-amber-700" : "text-slate-500"
                            }`}
                          >
                            Assigned To You
                          </div>
                          <div
                            className={`mt-1 text-sm font-semibold ${
                              pendingSecretarySteps.length > 0 ? "text-amber-950" : "text-slate-700"
                            }`}
                          >
                            {pendingSecretarySteps.length > 0
                              ? pendingSecretarySteps.length === 1
                                ? `${formatStepNumberLabel(pendingSecretarySteps)} is assigned to you`
                                : `${pendingSecretarySteps.length} steps are assigned to you`
                              : secretarySteps.length === 1
                                ? "Your assigned step is already completed"
                                : "Your assigned steps are already completed"}
                          </div>
                          <div
                            className={`mt-1 text-[11px] ${
                              pendingSecretarySteps.length > 0 ? "text-amber-700/90" : "text-slate-500"
                            }`}
                          >
                            {pendingSecretarySteps.length > 0
                              ? hasCurrentSecretaryStep
                                ? "Ready for your update now."
                                : `${formatStepNumberLabel(pendingSecretarySteps)} will open after earlier steps are done.`
                              : `${formatStepNumberLabel(secretarySteps)} completed.`}
                          </div>
                        </div>
                        ...
                        <span
                          ...
                          title={
                            pendingSecretarySteps.length > 0
                              ? `${pendingSecretarySteps.length} pending secretary-assigned ${
                                  pendingSecretarySteps.length === 1 ? "step" : "steps"
                                }`
                              : `${secretarySteps.length} secretary-assigned ${secretarySteps.length === 1 ? "step" : "steps"}`
                          }
                        >
                          {pendingSecretarySteps.length > 1
                            ? pendingSecretarySteps.length
                            : pendingSecretarySteps[0]?.number || secretarySteps[0]?.number}
                        </span>
                      </div>
                    </div>
                  )}
                          ...
                          {secretarySteps.length > 0 && (
                            <span
                              className={`inline-flex h-5 min-w-[1.25rem] items-center justify-center rounded-full px-1.5 text-[10px] font-bold ${
                                pendingSecretarySteps.length > 0 ? "bg-amber-500 text-white" : "bg-slate-200 text-slate-600"
                              }`}
                            >
                              {pendingSecretarySteps.length > 0 ? pendingSecretarySteps.length : secretarySteps.length}
                            </span>
                          )}
                        </button>
                      ) : (
