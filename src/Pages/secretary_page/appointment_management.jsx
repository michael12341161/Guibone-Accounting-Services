...
const APPOINTMENT_PAYMENT_FILTER_OPTIONS = [
...
export default function AppointmentManagement() {
  ...
  const canManageAppointments = canApproveAppointments || canDeclineAppointments;
    ...
    return (rows || [])
      .map((row) => {
        ...
        const isConsultation =
          String(row.Name || "").toLowerCase() === "consultation" ||
          /^\s*\[Type\]\s*Consultation\s*$/im.test(desc);
        ...
        return {
          ...
          client,
          service,
          paymentId: row.payment?.id ?? row.payment_id ?? null,
          meetingType,
          date,
          time,
          notes,
  ...
  const filtered = useMemo(() => {
    ...
    return normalized.filter((row) => {
      const matchesStatus =
        statusFilter === "all" ||
        normalizeAppointmentDecisionStatus(row.status) === statusFilter;
      const matchesPaymentStatus =
        paymentStatusFilter === "all" ||
        normalizePaymentStatusKey(row.paymentStatus) === paymentStatusFilter;
  ...
  }, [normalized, paymentStatusFilter, search, statusFilter]);

  const totalPages = Math.max(1, Math.ceil((filtered?.length || 0) / PAGE_SIZE));

  useEffect(() => {
    setCurrentPage(1);
  }, [paymentStatusFilter, search, statusFilter]);
  ...
  const openEditModal = (row) => {
    ...
    setEditForm({
      ...
      paymentId: paymentId > 0 ? String(paymentId) : "",
      paymentStatus: normalizePaymentStatusKey(row?.paymentStatus),
      hasPaymentRecord: paymentId > 0,
      meetingType: row?.type === "-" ? "" : String(row?.type || ""),
  ...
  const closeEditModal = () => {
    if (String(updatingId || "") === String(editForm.id || "")) {
      return;
    }
  ...
  const handleApproveAppointment = async (row) => {
    ...
    const isPaymentPaid = paymentStatusKey === "paid";
    const approveDisabled = disabled || !isPending || !isPaymentPaid;
    ...
    const result = await Swal.fire({
      ...
      text: `Approve ${row.client}'s appointment for ${row.service}?`,
      showCancelButton: true,
    ...
    await onUpdateStatus(row.id, "Approved");
  };

  const handleEditSubmit = async (event) => {
    event.preventDefault();

    const appointmentId = Number(editForm.id || 0);
    const paymentId = Number(editForm.paymentId || 0);
    const date = String(editForm.date || "").trim();
    const time = String(editForm.time || "").trim();
    const meetingType = String(editForm.meetingType || "").trim();
    const notes = String(editForm.notes || "").trim();
    const paymentStatus = normalizePaymentStatusKey(editForm.paymentStatus);

    if (appointmentId <= 0) {
      setEditError("Appointment could not be resolved.");
      return;
    }
    if (!date || !time) {
    ...
    try {
      ...
      const response = await api.post("/appointment_update.php", {
        appointment_id: appointmentId,
        date,
        time,
        meeting_type: meetingType || null,
        notes,
        ...(paymentId > 0 ? { payment_status: paymentStatus } : {}),
      });

      await refresh({ silent: true });
      showSuccessToast(response?.data?.message || "Appointment updated successfully.");
      setEditOpen(false);
      setEditForm(createInitialEditForm());
    } catch (requestError) {
      setEditError(
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, row) => {
        ...
        const editDisabled = disabled || !isPending;
        return (
          <div className="flex items-center justify-end gap-1">
            <IconButton
              ...
              title={
                !isPending
                  ...
                  : !isPaymentPaid
                    ? "Only paid appointments can be approved"
              ...
              onClick={() => {
                void handleApproveAppointment(row);
              }}
            >
            ...
            <IconButton
              ...
              aria-disabled={editDisabled || !canManageAppointments}
              className={!canManageAppointments ? "cursor-not-allowed opacity-60" : ""}
              onClick={() => {
            ...
            >
              <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" viewBox="0 0 24 24" fill="currentColor">
                <path d="M16.862 3.487a2.25 2.25 0 1 1 3.182 3.182L8.12 18.594 4 20l1.406-4.12L16.862 3.487Z" />
                <path d="M14.5 5.85 18.15 9.5" />
              </svg>
            </IconButton>
          </div>
        );
  ...
  return (
    <div className="space-y-4">
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col gap-2 lg:flex-row lg:flex-wrap lg:items-center">
            ...
            <select
              ...
              className="w-full rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 sm:w-40"
            ...
            <select
              ...
              className="w-full rounded-md border border-slate-300 bg-white px-3 py-1.5 text-xs outline-none focus:ring-2 focus:ring-indigo-500/20 sm:w-48"
              aria-label="Filter by payment status"
            >
              {APPOINTMENT_PAYMENT_FILTER_OPTIONS.map((option) => (
                <option key={option.value} value={option.value}>
                  {option.label}
                </option>
              ))}
      ...
      <Modal
        ...
        description="Update the appointment details before approval."
        size="sm"
        closeOnOverlayClick={String(updatingId || "") !== String(editForm.id || "")}
        footer={
          <>
            <Button
              type="button"
              ...
              disabled={String(updatingId || "") === String(editForm.id || "")}
            >
              Cancel
            </Button>
            <Button
              type="submit"
              form="secretary-appointment-edit-form"
              disabled={String(updatingId || "") === String(editForm.id || "")}
            >
              {String(updatingId || "") === String(editForm.id || "") ? "Saving..." : "Save Changes"}
            </Button>
          </>
        }
      >
        <form id="secretary-appointment-edit-form" onSubmit={handleEditSubmit} className="space-y-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div className="rounded-lg border border-slate-200 bg-slate-50/70 p-3">
              <div className="text-[11px] uppercase tracking-wide text-slate-500">Client</div>
              <div className="mt-1 text-sm font-medium text-slate-800">{editForm.client || "-"}</div>
            </div>
            <div className="rounded-lg border border-slate-200 bg-slate-50/70 p-3">
              <div className="text-[11px] uppercase tracking-wide text-slate-500">Service</div>
              <div className="mt-1 text-sm font-medium text-slate-800">{editForm.service || "-"}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  setEditForm((current) => ({ ...current, meetingType: event.target.value }));
                  if (editError) setEditError("");
                }}
                disabled={String(updatingId || "") === String(editForm.id || "")}
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 disabled:bg-slate-100"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(event) => {
                  setEditForm((current) => ({ ...current, paymentStatus: event.target.value }));
                  if (editError) setEditError("");
                }}
                disabled={
                  String(updatingId || "") === String(editForm.id || "") || !editForm.hasPaymentRecord
                }
                className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm outline-none focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 disabled:bg-slate-100"
              >
                {APPOINTMENT_PAYMENT_STATUS_OPTIONS.map((option) => (
                  <option key={option.value} value={option.value}>
                    {option.label}
                  </option>
                ))}
              </select>
              {!editForm.hasPaymentRecord ? (
                <p className="mt-1 text-xs text-slate-500">No payment record is available for this appointment yet.</p>
              ) : null}
            </div>
          </div>
