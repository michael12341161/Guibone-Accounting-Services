...
function getPersonDisplayName(person, fallback = "") {
  ...
  const fullName = joinPersonName([
    person?.first_name ?? person?.employee_first_name,
    person?.middle_name ?? person?.employee_middle_name,
    person?.last_name ?? person?.employee_last_name,
  ]);
  if (fullName) return fullName;
  const alt = String(person?.username || person?.email || person?.name || "").trim();
  return alt || fallback;
}
...
export default function SecretaryTaskManagement() {
  ...
  const canManageTaskWorkloadLimit = canViewTaskLimit;
    ...
    try {
      ...
      if (res?.data?.success) {
        ...
        const selectedAccountant =
          (Array.isArray(matchingAccountants) ? matchingAccountants : []).find(
        ...
        notifyTaskCreated({
          ...
          accountantName: getPersonDisplayName(selectedAccountant, ""),
  ...
  const filteredTodoTasks = useMemo(() => {
    return sortedTodoTasks.filter((task) => {
      ...
      if (!taskKey || archivedTaskIdSet.has(taskKey) || isAdminArchivedTask(task)) return false;
  ...
  return (
    <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-12 items-center hover:bg-slate-50">
                    ...
                    <div className="col-span-2 px-3 py-2 text-sm text-slate-700 flex items-center">
                      {(() => {
                        ...
                        if (!raw) return <span className="text-slate-400">Ã¢â‚¬â€</span>;
      ...
      <Modal
        ...
        description="Only tasks archived from secretary task management are shown here."
        size="lg"
        footer={
            ...
            <div className="rounded-xl border border-slate-200 bg-slate-50/70 p-4">
              <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                ...
                {canManageTaskWorkloadLimit ? (
                  ...
                  <div className="text-xs text-slate-500">You need Task Limit permission to update this limit.</div>
                )}
              </div>
            </div>
