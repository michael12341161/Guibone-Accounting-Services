...
export default function ClientAppointment() {
  ...
  const loadServices = async ({ silent } = { silent: false }) => {
    try {
      ...
      if (!silent) {
        applyServiceOptions([], serviceAccess);
      }
      if (isMountedRef.current && isCreateOpen && !silent) {
        setError(
  ...
  useEffect(() => {
    ...
    const intv = setInterval(() => {
      if (mounted && !isCreateOpen && !isRescheduleOpen) {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user?.client_id, user?.Client_ID, user?.username, isCreateOpen, isRescheduleOpen]);
