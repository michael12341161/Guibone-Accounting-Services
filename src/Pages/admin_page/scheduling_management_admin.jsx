...
export default function SchedulingManagementAdmin() {
                ...
                <div className="flex items-center justify-end gap-1">
                  ...
                  {canApproveConsultations ? (
                    <IconButton
                      ...
                      onClick={() => {
                        void handleApproveConsultation(row);
                      }}
                    >
  ...
  const handleApproveConsultation = async (row) => {
    const isPending = canUpdateConsultationDecision(row.status);
    const disabled = updatingId === row.id;

    if (disabled || !isPending || !canApproveConsultations) {
      return;
    }
    ...
    const result = await Swal.fire({
      ...
      text: `Approve ${row.client}'s consultation request?`,
      ...
      <Card compact>
        ...
        <CardContent className="space-y-4">
          <div className="flex flex-col gap-2 lg:flex-row lg:flex-wrap lg:items-center">
            ...
            >
              {CONSULTATION_STATUS_FILTER_OPTIONS.map((option) => (
