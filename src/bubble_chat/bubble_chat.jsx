...
async function requestChatUsers() {
...
async function requestConversation(partnerId) {
  const response = await api.get("chat.php", {
    params: {
      action: "messages",
...
function BubbleChatSurface({
  ...
  useEffect(() => {
    ...
    const loadUsers = async (quiet = false) => {
      ...
      try {
        const nextUsers = await requestChatUsers();
        if (cancelled) return;
  ...
  useEffect(() => {
    ...
    const loadConversation = async (quiet = false) => {
      ...
      try {
        const nextMessages = await requestConversation(selectedPartnerId);
        if (cancelled) return;
  ...
  }, [selectedPartnerId, showConversationPanel]);
