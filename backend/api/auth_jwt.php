...
function monitoring_get_jwt_issuer(): string
{
    ...
    $configuredAppUrl = trim((string)monitoring_read_auth_config_value('APP_URL', ''));
...
function monitoring_queue_jwt_response_header(array $user, ?int $issuedAtOverride = null): string
{
...
function monitoring_persist_session_user(array $user, bool $regenerateId = true, ?int $lastActivityAt = null): array
{
    ...
    if ($regenerateId) {
        if (!@session_regenerate_id(true) && !@session_regenerate_id(false)) {
            error_log('Unable to regenerate MONITORINGSESSID session id using save path: ' . session_save_path());
        }
    }
