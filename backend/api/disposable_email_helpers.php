<?php
$monitoringVendorAutoload = __DIR__ . '/../../vendor/autoload.php';
if (is_file($monitoringVendorAutoload)) {
    require_once $monitoringVendorAutoload;
}
...
function monitoring_email_is_blocked_by_temp_mail_rules(string $email): bool {
    ...
    if (monitoring_email_matches_managed_temp_mail_blocklist($normalizedEmail)) {
        return true;
    }

    if (class_exists(\Fgribreau\MailChecker::class)) {
        try {
            return \Fgribreau\MailChecker::isBlacklisted($normalizedEmail);
        } catch (Throwable $error) {
            error_log('MailChecker lookup skipped: ' . $error->getMessage());
        }
    }
