...
{
    return monitoring_request_is_https();
}
...
function monitoring_is_allowed_origin(string $origin): bool
{
    ...
    return monitoring_is_allowed_cors_origin($origin);
}
...
function monitoring_start_session(): void
{
    ...
    monitoring_prepare_session_storage();
    session_set_cookie_params([
        'lifetime' => 0,
        'path' => '/',
        'domain' => '',
        'secure' => monitoring_cookie_secure(),
        'httponly' => true,
        'samesite' => monitoring_cookie_same_site(),
    ]);

    if (!@session_start()) {
        error_log('Unable to start MONITORINGSESSID session using save path: ' . session_save_path());
    }
}
    ...
    if (ini_get('session.use_cookies')) {
        ...
        setcookie(
            ...
            [
                ...
                'samesite' => $params['samesite'] ?? monitoring_cookie_same_site(),
            ]
        );
    }
    ...
    try {
        ...
        $connection = new PDO(
            ...
            [
                ...
                PDO::ATTR_TIMEOUT => min(2, $dbConfig['timeout']),
            ]
        );
    } catch (Throwable $__) {
        $connection = null;
...
function monitoring_store_session_user(array $user): string
{
    $preparedUser = monitoring_persist_session_user($user, true);
    return monitoring_queue_jwt_response_header($preparedUser);
}
